...
public class BackPropMultiHiddenLayer {
    ...
    public double[] getLayer(int x, int scaler){
        double k[] = new double[layers[x].size];
        for (int i=0; i<layers[x].size; i++){
            k[i] = layers[x].units[i].output*scaler;
        }
        return k;
    }
    public double[][] getClassifierVariables(int x){
        double k[][] = new double[layers[x].size][3];
        for (int i=0; i<layers[x].size; i++){
            k[i] = new double[]{layers[x].units[i].weights[0], layers[x].units[i].weights[1], layers[x].units[i].bias};
        }
        return k;
    }
