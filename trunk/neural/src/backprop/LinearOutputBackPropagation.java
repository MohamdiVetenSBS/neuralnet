...
public class LinearOutputBackPropagation {
    protected LinearOutputBackPropagation.layer output_nodes;
	protected LinearOutputBackPropagation.layer hidden_nodes;
    ...
    public LinearOutputBackPropagation(final int input_size, final int hidden_size, final int output_size) throws Exception{
        if (input_size==0 || hidden_size==0 || output_size==0)
            throw new Exception("Bad network parameters!");
        this.input_size = input_size;
        this.hidden_size = hidden_size;
        this.output_size = output_size;
        this.hidden_nodes = new LinearOutputBackPropagation.layer(hidden_size, input_size);
        this.output_nodes = new LinearOutputBackPropagation.layer(output_size, hidden_size);
    ...
    protected class layer {
        ...
        protected LinearOutputBackPropagation.layer.neuron units[];
        public layer(final int size, final int input_size){
            this.size = size;
            this.input_size = input_size;
            units = new LinearOutputBackPropagation.layer.neuron[size];
            for (int i=0; i<size; i++){
                units[i] = new LinearOutputBackPropagation.layer.neuron(input_size);
            }
        }
